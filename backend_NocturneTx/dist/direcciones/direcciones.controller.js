"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.DireccionesController = void 0;
const openapi = require("@nestjs/swagger");
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const direcciones_service_1 = require("./direcciones.service");
const create_direccion_dto_1 = require("./dto/create-direccion.dto");
let DireccionesController = class DireccionesController {
    direccionesService;
    constructor(direccionesService) {
        this.direccionesService = direccionesService;
    }
    getAll(req) {
        return this.direccionesService.getAll(req.user.userId);
    }
    create(req, dto) {
        return this.direccionesService.create(req.user.userId, dto);
    }
    update(req, id, dto) {
        return this.direccionesService.update(req.user.userId, +id, dto);
    }
    delete(req, id) {
        return this.direccionesService.delete(req.user.userId, +id);
    }
};
exports.DireccionesController = DireccionesController;
__decorate([
    (0, common_1.Get)(),
    (0, swagger_1.ApiOperation)({ summary: 'Obtener todas las direcciones del usuario' }),
    openapi.ApiResponse({ status: 200, type: [require("./entities/direccion.entity").Direccion] }),
    __param(0, (0, common_1.Request)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], DireccionesController.prototype, "getAll", null);
__decorate([
    (0, common_1.Post)(),
    (0, swagger_1.ApiOperation)({ summary: 'Crear nueva dirección' }),
    openapi.ApiResponse({ status: 201, type: require("./entities/direccion.entity").Direccion }),
    __param(0, (0, common_1.Request)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, create_direccion_dto_1.CreateDireccionDto]),
    __metadata("design:returntype", void 0)
], DireccionesController.prototype, "create", null);
__decorate([
    (0, common_1.Patch)(':id'),
    (0, swagger_1.ApiOperation)({ summary: 'Actualizar dirección' }),
    openapi.ApiResponse({ status: 200, type: require("./entities/direccion.entity").Direccion }),
    __param(0, (0, common_1.Request)()),
    __param(1, (0, common_1.Param)('id')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String, create_direccion_dto_1.CreateDireccionDto]),
    __metadata("design:returntype", void 0)
], DireccionesController.prototype, "update", null);
__decorate([
    (0, common_1.Delete)(':id'),
    (0, swagger_1.ApiOperation)({ summary: 'Eliminar dirección' }),
    openapi.ApiResponse({ status: 200, type: require("./entities/direccion.entity").Direccion }),
    __param(0, (0, common_1.Request)()),
    __param(1, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", void 0)
], DireccionesController.prototype, "delete", null);
exports.DireccionesController = DireccionesController = __decorate([
    (0, swagger_1.ApiTags)('direcciones'),
    (0, swagger_1.ApiBearerAuth)('JWT-auth'),
    (0, common_1.Controller)('direcciones'),
    __metadata("design:paramtypes", [direcciones_service_1.DireccionesService])
], DireccionesController);
//# sourceMappingURL=direcciones.controller.js.map